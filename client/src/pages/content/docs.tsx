...
import { Search, Plus, FileText, ChevronRight, Folder, MoreVertical } from "lucide-react";
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {Object.entries(groupedItems).map(([category, docs]) => (
               <div key={category} className="mb-4">
                  <h3 className="px-2 mb-1 text-xs font-medium text-muted-foreground uppercase tracking-wider">{category}</h3>
                  <div className="space-y-0.5">
                     {docs.map(doc => (
                        ...
                        >
                           ...
                           {selectedDoc?.id === doc.id && <ChevronRight className="h-3 w-3 opacity-50" />}
                        </button>
                     ))}
                  </div>
               </div>
              ...
              <>
                 <div className="flex items-center justify-between">
                    <div className="space-y-1">
                       <h2 className="text-2xl font-bold tracking-tight">{selectedDoc.title}</h2>
                       <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          <span className="bg-muted px-1.5 py-0.5 rounded text-xs">{selectedDoc.category}</span>
                          <span>•</span>
                          <span className="font-mono text-xs">{selectedDoc.slug}</span>
                       </div>
                    </div>
